...
{
    public class DataExtractorPointer16LittleEndian : IDataExtractor
    {
        ...
        [JsonIgnore]
        public string Name => LibResource.dataExtractorPointer16LittleEndian;

        public void Extract(Project project, DataSettings settings)
        {
            settings.DataFile.Instance.Clear();
            ...
            using (var romFile = File.OpenRead(romPath))
            {
                ...
                foreach(var data in extractedData)
                {
                    ...
                    DataEntry newEntry = ReferenceDatabase.Instance.CreateNew<DataEntry>();
                    newEntry.EntryName = data.ToString();
                    newEntry.Original = lineBuilder.ToString();

                    settings.DataFile.Instance.DataEntries.Add(newEntry);
                ...
                {
                    settings.DataFile.Instance.Pointers.Add(new DataPointer
                    {
                        PointerIndex = entry.ExtractedData.Pointers.IndexOf(entry.Pointer),
                        Pointer = new RefObjectPtr<DataEntry>() { Instance = entry.ExtractedData.DataEntry }
                    });
                }
            }
